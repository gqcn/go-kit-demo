...
- `manifest`：项目交付清单。非二进制编译打包的资源文件，例如镜像编译文件、部署文件模板、配置文件模板。
...
# 公司团队会议讨论结果记录
1. 严格确定每一层的职责：
    - 所有的业务逻辑写到`service`中，数据校验也是在具体的`service`逻辑中做。
    - 所有的数据库操作写到`repository`中，不允许在`repository`外部执行数据库操作。
    - `repository`层有独立的输入输出定义，不允许往该层透传上层的输入输出数据结构定义。
    - `service`层可以直接使用`api/grpc`接口的输入输出数据结构定义。
    - `endpoint`负责调用`service`实现具体的接口，它只是实现业务接口数据结构与`service`接口数据结构的转换。
2. `service/repository`层必须要定义接口：
    - 自顶向下接口设计的需要，先按照业务需求定义接口再做具体实现。接口与实现分离。
    - 模块实现解耦的需要。比如`repository`存储层，具体存储实现发生变化，接口不用变化。
    - 最后，不太重要一点，是写单元测试`mock`的需要。
3. 其他一些讨论到的细节点：
    - 未来考虑在项目中引如`Golang`的`internal`特性。
    - 未来考虑通过自动化的数据模型以及数据对象及字段结构生成来避免字段硬编码问题。
    - 目前暂不考虑通过版本号(`v1/v2`方式)来预控接口兼容性问题。
4. 目前的习题二代码需要调整为以上所诉的规范。
